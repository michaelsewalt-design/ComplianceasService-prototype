...
/* ── log submission to compliance register ── */
async function logSubmission(submission, aiAnalysis) {
  const token = sessionStorage.getItem('ir_token');
  const response = await fetch(LOG_ENDPOINT, {
    ...
    body: JSON.stringify({ submission, aiAnalysis }),
  });
  if (!response.ok) throw new Error('Log endpoint returned ' + response.status);
...
function buildEmailBody(formData, aiAnalysis) {
  ...
  lines.push('\n══════════════════════════════════════════');
  lines.push('DISCLAIMER — AI-GENERATED CONTENT');
  lines.push('══════════════════════════════════════════');
  lines.push('This compliance review has been generated by an artificial intelligence model (Claude, Anthropic) and is provided for informational and preliminary screening purposes only. It does not constitute legal, regulatory, or professional advice.');
  lines.push('');
  lines.push('All decisions, actions, and conclusions based on this report remain the sole responsibility of the receiving entity and its authorized decision-makers. No rights, obligations, or entitlements may be derived from the content of this report.');
  lines.push('');
  lines.push('The AI analysis may contain inaccuracies, omissions, or outdated information. Users are strongly advised to independently verify all findings and consult qualified legal or compliance professionals before acting on any information contained herein.');
...
async function submitForm() {
  ...
  try {
    ...
    var disclaimerEl = document.getElementById('disclaimer-block');
